...
import utils
from extensions import detection, memory_handler
...
def train(data, model, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(len(nodes) // args.batch_size):
            ...
            batch_labels = torch.LongTensor(data.labels[np.array(batch_nodes)]).to(
                args.device
            )
...
def run(args, t):
    ...
    logging.info(
        "Number of ALL Nodes / Edges: "
        + str(len(data.adj_lists))
        + " / "
    ...
    logging.info(
        ...
        + str(data.data_size)
        + "; Train size: "
        + str(len(data.train_nodes))
        + "; Valid size: "
        + str(len(data.valid_nodes))
    )

    if args.new_ratio > 0.0 and t > 0:
        data.train_new_nodes_list = detection.detect(data, t, args)
        data.train_nodes = (
            list(set(data.train_nodes + data.train_new_nodes_list))
            if len(data.train_new_nodes_list) > 0
            else data.train_nodes
        )
    else:
        pass
    ...
    # Sage layers
    layers = (
        [data.feature_size] + [args.embed_size] * args.num_layers + [data.label_size]
    )
    ...
    if t > 0:
        ...
        ewc_model.register_ewc_params(
            important_nodes_list,
            torch.LongTensor(data.labels[important_nodes_list]).to(args.device),
        )
    else:
        if args.memory_size > 0:
    ...
    if args.memory_size > 0:
        ...
        memory_h.update(
            data.train_nodes, x=train_output, y=data.labels, adj_lists=data.adj_lists
        )
...
if __name__ == "__main__":
    ...
    print(
        np.round(avg_ans[0] / t_num, 6),
        np.round(avg_ans[1] / t_num, 6),
        np.round(avg_ans[2] / t_num, 6),
    )
