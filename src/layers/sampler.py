...
class Sampler(object):
    ...
    def sample_neighbors(self, nodes, num_sample=10):
        ...
        if not num_sample is None:
            samp_neighs = [
                set(random.sample(to_neigh, num_sample))
        ...
        nodes_2_idxs_mask = Variable(
            torch.zeros(len(samp_neighs), len(unique_neighs_2_idxs))
        )
        column_indices = [
            unique_neighs_2_idxs[n] for samp_neigh in samp_neighs for n in samp_neigh
        ]
        row_indices = [
