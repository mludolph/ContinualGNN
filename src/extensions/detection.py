...
def detect_simple(data, t, args):
    ...
    h_pre = get_h(
        model, data.train_cha_nodes_list + data.train_old_nodes_list, t - 1, args
    )
    ...
    new_nodes = np.array(data.train_cha_nodes_list + data.train_old_nodes_list)[
        np.where(delta_h > threshold)[0]
    ]
...
def detect_bfs(data, t, args):
    ...
    new_nodes = list(
        set(new_nodes).intersection(
            set(data.train_cha_nodes_list + data.train_old_nodes_list)
        )
    )
...
class AdjListsHandler(StreamDataHandler):
    def load(self, data_name, t):
        ...
        end_time = t
        for tt in range(
            0, len(os.listdir(os.path.join("../data", data_name, "stream_edges")))
        ):
