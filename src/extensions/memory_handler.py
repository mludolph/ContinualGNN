...
class MemoryHandler(object):
    ...
    def _update_class(self, nodes, y, importance):
        ...
        for i in self.data_size_per_class:
            self.memory_size_per_class[i] = int(
                self.data_size_per_class[i] / self.data_size * self.memory_size
            )
        ...
        for i, node in enumerate(nodes):
            if node in self.memory_per_class[y[i]]:
                ...
                self.memory_per_class_log[y[i]] += [
                    (node, int(y[i]), self.clock, importance[i])
                ]
            else:
                prob = (
                    self.p
                    * self.memory_size_per_class[y[i]]
                    / self.data_size_per_class[y[i]]
                    * (1 + self.alpha * importance[i])
                )
                if random.random() > prob:
                    continue
                replace_idx = random.randint(0, len(self.memory_per_class[y[i]]) - 1)
                self.memory_per_class[y[i]][replace_idx] = node
                self.memory_per_class_log[y[i]][replace_idx] = (
                    node,
                    int(y[i]),
                    self.clock,
                    float(importance[i]),
    ...
    def _compute_node_importance(self, nodes, labels, adj_lists):
        ...
        for node in nodes:
            ...
            neighbor_labels = np.array(
                [node_label]
                + [labels[neighbor] for neighbor in neighbors if labels[neighbor] != -1]
            )
            importance += [
                np.sum(neighbor_labels != node_label) / neighbor_labels.shape[0]
            ]
